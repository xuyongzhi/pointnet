...
#ETH_training_sortedAh5_0d5_folder = '/short/dh01/yx2146/Dataset/ETH_Semantic3D_Dataset/training/sorted_A_h5_0.5block'
#ETH_training_sortedAh5_10b_folder = '/short/dh01/yx2146/Dataset/ETH_Semantic3D_Dataset/training/sorted_A_h5_10block'
...
class Sorted_H5f():
    ...
    def copy_root_summaryinfo_from_another(self,h5f0):
        ...
        if 'total_row_N' in h5f0.sorted_h5f.attrs:
            self.sorted_h5f.attrs['total_row_N'] = h5f0.sorted_h5f.attrs['total_row_N']
        if 'total_block_N' in h5f0.sorted_h5f.attrs:
            self.sorted_h5f.attrs['total_block_N'] = h5f0.sorted_h5f.attrs['total_block_N']
        self.get_summary_info()
    ...
    def get_block_scope_from_k(self,block_k):
        ...
        block_scope_k[0,:] = i_xyz * self.block_step + self.xyz_min
        block_scope_k[1,:] = (i_xyz+1) * self.block_step + self.xyz_min
        return block_scope_k
    def check_xyz_scope_k(self,block_k):
        dset = self.sorted_h5f[str(block_k)]
        scope_anno = dset.attrs['xyz_scope']
        scope_k = self.get_block_scope_from_k(block_k)

        e = scope_anno-scope_k
        e = np.linalg.norm(e)
        if e > 1e-5:
...
class OUTDOOR_DATA_PREP():
    ...
    def merge_blocks_to_new_step(self,base_file_name,larger_step,larger_stride):
        ...
        new_name = base_file_name.split('_step_0d5_stride_0d5.h5')[0] + '_step_' + str(larger_step[0]) + '_stride_' + str(larger_stride[0]) + '.hdf5'
    ...
    def get_block_index_multi(self,raw_buf,block_step,block_dims_N):
        ...
        for i in range(0,raw_buf.shape[0],step):
            ...
            p = mp.Process(target=self.get_block_index_subbuf,args=(raw_buf[i:end,0:3],block_ks,i,block_step,block_dims_N) )
            p.start()
            pool.append(p)
        for p in pool:
            p.join()
    ...
    def get_block_index_subbuf(self,sub_buf_xyz,block_ks,i_start,block_step,block_dims_N):
    ...
    def sort_to_blocks(self,file_name):
        ...
        with h5py.File(blocked_file_name,'w') as self.h5f_blocked,  h5py.File(file_name,'r') as h5_f:
            ...
            block_dims_N = self.raw_h5f.get_block_dims_N(block_step)
            blocks_N = block_dims_N[0] * block_dims_N[1] * block_dims_N[2]
            ...
            for k in range(0,self.raw_h5f.total_row_N,row_step):
                ...
                sorted_buf_dic={}
                self.sort_buf(raw_buf,k,sorted_buf_dic,block_step,block_dims_N)

                #t2_1_k = time.time()
                self.h5_write_buf(sorted_buf_dic,block_step,block_dims_N)
    ...
    def sort_buf(self,raw_buf,buf_start_k,sorted_buf_dic,block_step,block_dims_N):
        #t0 = time.time()
        IsMulti = False
        if IsMulti:
            block_ks = self.get_block_index_multi(raw_buf,block_step,block_dims_N)
        else:
    ...
    def h5_write_buf(self,sorted_buf_dic,block_step,block_dims_N):
        for key in sorted_buf_dic:
            sorted_buf_dic[key] = np.concatenate(sorted_buf_dic[key],axis=0)
        for block_k in sorted_buf_dic:
            dset_k =  self.get_blocked_dset(block_k,block_step,block_dims_N)
    ...
    def Add_block_dims_N_attrs(self):
        ...
        for raw_fn in ETH_raw_h5_glob:
            ...
            if os.path.exists(sorted_name):
                ...
                with h5py.File(raw_fn,'r') as raw_f, h5py.File(sorted_name,'a') as sorted_f:
