...
def Do_Check_xyz():
    ...
    #for fn in fnl:
        ...
        fn_s = os.path.join( GLOBAL_PARA.ETH_A_stride_1_step_1,'bildstein_station5_stride_1_step_1.h5')
        print('checking equal and  xyz scope of file: ',fn_s)
        #with h5py.File(raw_fn,'r') as h5f:
        with h5py.File(fn_s,'r') as sh5f:
                sorted_h5f = Sorted_H5f(sh5f,fn_s)
...
class OUTDOOR_DATA_PREP():
 ...
 #   h5_chunk_row_step =  h5_chunk_row_step_10M
    ...
    def Do_merge_blocks(self):
        ...
        file_list = glob.glob( os.path.join(GLOBAL_PARA.ETH_A_stride_1_step_1,   'bildstein_station5_stride_1_step_1_sub_m80_m5.h5') )
        #file_list = glob.glob( os.path.join(GLOBAL_PARA.ETH_A_step_10_stride_10,   '*_blocked.h5_sorted_step_10_stride_10.hdf5') )
        block_step = np.array([1,1,1])*4
